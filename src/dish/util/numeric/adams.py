import numpy as np
from dish.util import math_util

from dish.util.math_util.linear_algebra import matmul_pointwise
from dish.util.LUTs import AM_taylor_coeffs
try:
    from dish.util.numeric.adams_f import adams as adams_f_subroutine
    _NO_FORTRAN = False
...
def adams_moulton_coefficients(order) -> np.ndarray:
    ...
    for n in range(1, order + 1):
        ...
        f_nk_coeffs = np.array([1] + [(-1) ** k * math_util.factorial(n) // (math_util.factorial(n - k) * math_util.factorial(k))
